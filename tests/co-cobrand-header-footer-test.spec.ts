...
test.describe('CO cobrand, header and footer links Test', () => {
  ...
  test('verify header and footer links are working', async ({ page }) => {
    ...
    const headerLinks = [
      ...
      {
        text: 'LOGIN',
        url: 'https://search.211colorado.org/users/sign_in?_gl=1*bsbeov*_ga*MTIxNDcxNjc3OS4xNjg2MjQ1MDA3*_ga_2SYE2QY2YJ*MTY4OTAxODY0Ni43LjEuMTY4OTAxODkwMC42MC4wLjA',
      },
    ];
    ...
    for (const link of headerLinks) {
      ...
      const [newPage] = await Promise.all([page.waitForEvent('popup'), linkElement.click()]);
